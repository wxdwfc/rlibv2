...
inline IOStatus send_request(const MacID &id, uint16_t req_type,
                             const Buf_t &req, Buf_t &reply,
                             const struct timeval &timeout) {
  IOStatus ret = SUCC;

  RequestHeader req_h = {.req_type = req_type,
                         .req_payload = static_cast<uint16_t>(req.size())};
