...
int main(int argc, char **argv) {
    ...
    RDMA_LOG(rdmaio::WARNING) << "Pingping server listenes at localhost:" << FLAGS_port;
    

    Arc<RNic> nic = RNic::create(RNicInfo::query_dev_names().at(FLAGS_use_nic_idx)).value();
    ...
    RDMA_ASSERT(ctrl.opened_nics.reg(FLAGS_reg_nic_name, nic));
    {
        ...
        RDMA_ASSERT(ctrl.registered_mrs.create_then_reg(
            FLAGS_reg_mem_name, Arc<RMem>(new RMem(1024 * 1024)),
